...
/**
 * BOM服务（版本化主数据：草稿 → 发布 → 冻结）
 *
 * @author xmes
 */
public interface MdBomService extends IService<MdBom> {
	 ...
	 * @return 分页结果
	 */
	Page<BomDTO> pageDetail(Page<MdBom> page, Long materialId, String status);
	 ...
	 * @param dto BOM
	 */
	void saveDraft(BomDTO dto);

	/**
	 * 修改草稿（全量替换明细行，仅草稿可改）
	 * @param dto BOM
	 */
	void updateDraft(BomDTO dto);
